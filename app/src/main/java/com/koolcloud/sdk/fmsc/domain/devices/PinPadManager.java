package com.koolcloud.sdk.fmsc.domain.devices;
...
public class PinPadManager {
    private static JSONObject mtransData;
    ...
    public static PinPadManager getInstance(JSONObject transData, Handler handler) {
    ...
    public void start() {
        brhKeyIndex = mtransData.optString("brhKeyIndex", "00");
        ...
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                String pan = mtransData.optString("pan");
                if (TextUtils.isEmpty(pan)) {
                    pan = "0000000000000000000";
                }
                byte[] bytes_pan = pan.getBytes();
                byte[] pinBlock = new byte[8];
                int keyIndex = Integer.parseInt(brhKeyIndex);
                PinPadInterface.selectKey(2, keyIndex, 0, 1);
                String actionPurpose = mtransData.optString("actionPurpose");
                if (!actionPurpose.equals("Balance")) {
                    String amount = mtransData.optString("transAmount");
                    if (!TextUtils.isEmpty(amount)) {
                        String text = amount;
                        String language = Locale.getDefault().getLanguage();
                        if (!TextUtils.isEmpty(language) && language.equals(Constant.LANGUAGE_CHINESE)) { //中文的排版
                            ...
                            PinPadInterface.showText(0, bytes_text_line_1, bytes_text_line_1.length, 1);
                            PinPadInterface.showText(1, bytes_text_line_2, bytes_text_line_2.length, 1);
                        } else {
                ...
                if (pwdInputResult < 0) {
                    ...
                    pwd = StringUtils.hexString(pinBlock);
                    Boolean needAuthCode = mtransData.optBoolean("needAuthCode");
                    if (needAuthCode) {
                        ...
                        if (authInputResult < 0) {
                            ...
                            authCode = StringUtils.hexString(auth);
                        }
                    }
                }
                PinPadInterface.close();// 关闭占用
                try {
                    mtransData.put("isCancelled", isCancelled);
